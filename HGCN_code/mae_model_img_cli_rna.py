...
class MixerBlock(nn.Module):
    def __init__(self,dim1,dim2):#dim1=2=训练模态数,dim2=512=输出类数目
        super(MixerBlock,self).__init__() 
        
        self.norm = LayerNorm(dim1)#2
        self.norm = LayerNorm(dim2)#512
    ...
    def forward(self,x): 
        import pdb
        # pdb.set_trace()
        # x = x.transpose(0,1)#转置或者改LayerNorm使得norm运算正确
        y = self.norm(x)
        y = y.transpose(0,1)
        y = self.mix_mip_1(y)
        y = y.transpose(0,1)
