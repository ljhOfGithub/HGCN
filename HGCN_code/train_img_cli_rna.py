...
from mae_model_img_cli_rna import fusion_model_mae_2
...
folder = 'mysplit-4'
foldername=f"{'../result_metrics'}/{folder}"
import os
...
def main(args): 
    ...
    if cancer_type == 'lihc': 
        ...
        # all_data = torch.load('/home/jupyter-ljh/data/mydata/HGCN-main/all_data_cli_img.pt')
        all_data = torch.load('/home/jupyter-ljh/data/mydata/HGCN-main/all_data_cli_img_rna.pt')
        all_data = Data.from_dict(all_data)
        # seed_fit_split = joblib.load('/home/jupyter-ljh/data/mydata/HGCN-main/split.pkl')
        seed_fit_split = joblib.load('/home/jupyter-ljh/data/mydata/HGCN-main/data_split/split.pkl') #方便调试
    ...
    for seed in range(start_seed,start_seed+repeat_num):
        ...
        for train_index, test_index in kf.split(patients,kf_label):
            ...
            for epoch in range(epochs):
                ...
                t_test_loss,test_ci,test_img_ci,test_rna_ci,test_cli_ci = prediction(all_data,model,test_data,patient_and_time,patient_sur_type,args) #得到test_ci，不是test_fold_ci里面的
            ...
            t_test_loss,test_ci,_,_,_ = prediction(all_data,t_model,test_data,patient_and_time,patient_sur_type,args) #得到test_ci
            

            test_fold_ci.append(test_ci) #添加test_ci
            ...
            with torch.no_grad():
                for id in test_data:  
                    ...
                    # print(data.sur_type.cpu().detach().numpy()[0],one_x.cpu().detach().numpy()[0],patient_and_time[id]) #打印的信息有点占命令行
                    one_test_feature[id] = {}
                    ...
                    # if True:
                        ...
                        # for i,two_type_name in enumerate([['img','cli']]):
            ...
            for i,type_name in enumerate(['img','rna','cli']): 
                ...
                try:
                    ...
                    print(e)
            if True:#改成img和cli
            # if len(args.train_use_type) == 3:#改成img和cli
                for i,type_name in enumerate([['img','rna'],['img','cli'],['rna','cli']]): 
                # for i,type_name in enumerate([['img','cli']]): 
            ...
            print('all ci:',test_ci) #当前fold即split的平均ci

            # torch.save(t_model.state_dict(), 'your path'+sys_time.strftime('%Y-%m-%d')+label+'_'+str(seed)+'_'+str(n_fold)+'.pth')
            torch.save(t_model.state_dict(), '/home/jupyter-ljh/data/mydata/HGCN-main/result/'+sys_time.strftime('%Y-%m-%d %H:%M:%S')+label+'_'+str(seed)+'_'+str(n_fold)+'.pth')#添加小时
            # torch.save(t_model.state_dict(), '/home/jupyter-ljh/data/mydata/HGCN-main/result_bug/'+sys_time.strftime('%Y-%m-%d')+label+'_'+str(seed)+'_'+str(n_fold)+'.pth')
            del model, train_data, test_data, t_model
            
        filename=f"{'../result_metrics'}/{folder}/test_val.txt"
        ...
        all_fold_test_ci.append(test_fold_ci) #每一个fold都是一个list的元素
    ...
    filename=f"{'../result_metrics'}/{folder}/summary.txt"
    with open(filename, 'a') as file:
        sys.stdout = file
        print('summary :')
        print(label)  

        print('fusion test fold ci') #每一行都是一个split的test fold ci
...
def get_params():
    ...
    # parser.add_argument("--train_use_type", type=list, default=['img','cli'], help='train_use_type,Please keep the relative order of img, rna, cli')
    ...
    # parser.add_argument("--repeat_num", type=int, default=5, help="Number of repetitions of the experiment") #随机种子的范围
    parser.add_argument("--repeat_num", type=int, default=1, help="Number of repetitions of the experiment") #随机种子的范围 方便调试 由于range必须大于0
    ...
    parser.add_argument("--if_fit_split", default=True, help="fixed division/random division") #要用自己的split必须改为true
