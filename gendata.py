...
'''
patients = joblib.load('patients.pkl') #全部的病人950
'''
...
#         print(x,len(t_cli_feas[x]))
    ...
    maxx = np.max(zz)
    minn = np.min(zz)
    for x in t_cli_feas:#将t_cli_feas[x][i]转换为float
        t_cli_feas[x][i] = (float(t_cli_feas[x][i])-(maxx+minn)/2)/(maxx-minn)*2 
...
for id in patients:
    ...
    # data=Data(x_img=node_img,x_rna=None,x_cli=node_cli,sur_type=sur_type,data_id=data_id,data_type=t_data_type,edge_index_model=None,edge_index_image=edge_index_image,edge_index_rna=None,edge_index_cli=edge_index_cli) 
    data=Data(x_img=node_img,x_rna=node_rna,x_cli=node_cli,sur_type=sur_type,data_id=data_id,data_type=t_data_type,edge_index_model=None,edge_index_image=edge_index_image,edge_index_rna=edge_index_rna,edge_index_cli=edge_index_cli) 
    all_data[id] = data
    num+=1
    # print(data)
    print(num)
...
# torch.save(all_data, './HGCN_code/all_data_cli_img.pt') #方便兼容，使用torch.save
torch.save(all_data, './all_data_cli_img_rna.pt') #添加rna
